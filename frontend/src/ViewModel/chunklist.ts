...
function makeModifyMutator(id: string, chunkContent: ChunkContent, updatedAt: number): ChunkListStateMutator {
  return (state: ChunkListState) => {
    const chunks = [...state];
    const i = chunks.findIndex(c => c.id === id);
    if (i < 0) {
...
export class ChunkListViewModel extends EventTarget {
  ...
  updateOnNotification(notification: RPCNotification): void {
    if (notification.method === "chunk.refresh") {
      ...
      switch (kind) {
        case "chunk.create":
          mutator = makeCreateMutator(method.chunkId, updatedAt, method.position, method.chunkContent);
          break;
        case "chunk.delete":
        ...
        case "chunk.move":
          ...
          break;
        case "chunk.modify":
          mutator = makeModifyMutator(method.chunkId, method.chunkContent, updatedAt);
          break;
        default: {
  ...
  apply(mutator: ChunkListStateMutator, updatedAt: number, seq: number, refresh = true): void {
    ...
    for (const chunk of this.history.current) {
      ...
      if (ch === undefined) {
        appiledChunks.push(new ChunkViewModel(this, chunk, currentUpdatedAt));
      } else {
