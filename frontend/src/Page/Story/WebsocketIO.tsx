...
import { Input, Snackbar, Grid, Box, Tabs, Tab } from "@mui/material";
import { RPCMessageManager, openDocument, RPCErrorWrapper } from "../../Model/mod";
...
function OpenDocumentForm() {
    ...
    return <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
            <AsyncLoadingButton onClick={async () => {
                try{
...
export function WebsocketIO() {
    ...
    return <div>
        <Box sx={{ maxWidth: 750, margin: "auto" }}>
            ...
            }}>{connected ? "Disonnect" : "Connect"}</AsyncLoadingButton>
        </Box>
        {connected && <Grid container columns={2}>
            <Grid item xs={6}>
                <MessageSendForm />
            </Grid>
            <Grid item xs={6}>
        ...
        <Snackbar anchorOrigin={{ horizontal: "center", vertical: "top" }} open={open} onClose={() => { setOpen(false) }}
            message={message}
        />
    </div>
